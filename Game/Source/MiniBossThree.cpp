...
Mini_Boss_Three::Mini_Boss_Three(int x, int y) : Entity(x, y)
{
	MaxHp = 40;
	EntityHP = MaxHp;
	EntityAP = 8;
	...
	Mini_Boss_Three_List.add(app->physics->CreateZombieStandartBox(position.x, position.y, 25, 10));



}

void Mini_Boss_Three::Update(float dt)
{
	...
	if (EntityHP > 0)
	{
		...
		if (app->player->pauseMenu == true)
		{
			...
			Mini_Boss_Three_List.end->data->GetPosition(NewPosition.x, NewPosition.y);
			...
			if (entityState == GameState::OutOfCombat)
			{
				...
				if (app->inventoryMenu->ActiveRadioAlive == true)
				{
					if (position.DistanceTo(app->inventoryMenu->ActiveRadioPosition) < 2650)
					{
						...
						Mini_Boss_Three_List.end->data->GetPosition(position.x, position.y);
						currentAnim = &idleDownAnim_Enemy1;
						...
						if (position.x > app->inventoryMenu->ActiveRadioPosition.x)
						{
							...
							Mini_Boss_Three_List.end->data->body->SetLinearVelocity({ -0.5f, 0.0f });
						}
						if (position.x < app->inventoryMenu->ActiveRadioPosition.x)
						{
						...
						if (position.y > app->inventoryMenu->ActiveRadioPosition.y)
						{
							...
							Mini_Boss_Three_List.end->data->body->SetLinearVelocity({ 0.0f, -0.5f });
						}
						if (position.y < app->inventoryMenu->ActiveRadioPosition.y)
						{
						...
						if ((app->inventoryMenu->ActiveRadioPosition.y > position.y) && (app->inventoryMenu->ActiveRadioPosition.x > position.x))
						{
							...
							Mini_Boss_Three_List.end->data->body->SetLinearVelocity({ 0.5f, 0.5f });
						}
					...
					else
					{
						...
						Mini_Boss_Three_List.end->data->body->SetLinearVelocity({ 0.0f,0.0f });
			...
			if (entityState == GameState::InCombat)
			{
				...
				if (EntityHP > 0)
				{
					...
					if (entityTurn == TurnState::StartOfTurn)
					{
						...
						currentAnim = &idleDownAnim_Enemy1;
						currentAnim->loop = false;
						Mini_Boss_Three_List.end->data->body->SetLinearVelocity({ 0.0f, 0.0f }); //movimiento contrario a la direccion del jugador cuando nos chocamos con el
