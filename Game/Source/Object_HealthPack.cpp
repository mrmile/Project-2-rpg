...
Object_HealthPack::Object_HealthPack(int x,int y) : Entity(x,y)
{	
	EntityHP = 1;
	EntityAP = 5;
	EntityMP = 3;
	...
	collider = app->collisions->AddCollider({ position.x, position.y, 25, 56 }, Collider::Type::ITEM_HEALTH_PACK, (Module*)app->entity_manager);

}

bool Object_HealthPack::Update(float dt)
{
