...
Object_LongScopeGun::Object_LongScopeGun(int x,int y) : Entity(x,y)
{	
	EntityHP = 1;
	EntityAP = 5;
	EntityMP = 3;
	...
	collider = app->collisions->AddCollider({ position.x, position.y, 25, 56 }, Collider::Type::ITEM_LONG_RANGE_GUN, (Module*)app->entity_manager);

}

bool Object_LongScopeGun::Update(float dt)
{
